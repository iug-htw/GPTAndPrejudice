...
def build_neuron_concept_map(layer=1, base_dir="sae_probing"):
    ...
    Saves to: sae_probing/mappings/neuron_concept_primary_secondary_l{layer}.csv
    """
    path = os.path.join(base_dir, "mappings", f"neuron_label_assoc_l{layer}.csv")
    df = pd.read_csv(path)
    ...
    out_path = os.path.join(base_dir, "mappings", f"neuron_concept_primary_secondary_l{layer}.csv")
