...
def run_promts_4(model, generate_function):
    ...
    with open('generated_test_results_60_09_100.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['female_prompt', 'female_result', 'male_prompt', 'male_result'])
        writer.writeheader()
        writer.writerows(results)
    
    print(f"Saved {len(results)} results to generated_test_results_60_09_100.csv")
    return results
